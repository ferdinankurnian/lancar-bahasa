                        ...
                        <div class="row">
                            @if (Session::get('payable_amount') > 0)
                                ...
                                @if ($midtrans_credentials->status == '1')
                                    <div class="col-lg-3 col-6 col-sm-4">
                                        <a href="javascript:;" class="wsus__single_payment" onclick="midtransPayment()">
                                            <img src="{{ asset($midtrans_credentials->image) }}"
                                                alt="Midtrans method" class="img-fluid w-100">
                                        </a>
                                    </div>
                                @endif
                            @else
    ...
    @if ($midtrans_credentials->status == '1')
        ...
        <script type="text/javascript">
            ...
            function midtransPayment() {
                ...
                // Make an AJAX request to your backend to get the Snap Token
                $.ajax({
                    type: 'POST',
                        ...
                        if (response.snap_token) {
                            snap.pay(response.snap_token, {
                                onSuccess: function(result) {
                                    ...
                                    window.location.href = "{{ route('midtrans.finish') }}"; // Redirect to finish page
                                },
                                onPending: function(result) {
                                    /* You may add your own implementation here */
                                    // alert("wating your payment!");
                                    window.location.href = "{{ route('midtrans.unfinish') }}"; // Redirect to unfinish page
                                },
                                onError: function(result) {
                                    /* You may add your own implementation here */
                                    // alert("payment failed!");
                                    window.location.href = "{{ route('midtrans.error') }}"; // Redirect to error page
                                },
                                onClose: function() {
                                    /* You may add your own implementation here */
                                    // alert('customer closed the popup without finishing the payment');
                                    toastr.error("{{ __('Payment popup closed without finishing the payment.') }}");
                                }
                            });
                        } else {
