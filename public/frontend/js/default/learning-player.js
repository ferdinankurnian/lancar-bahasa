...
function updateNavigator() {
    ...
    const currentIndex = parseInt(activeLesson.attr('data-index'));
    const totalLessons = $('.lesson-item').length;
    const completedLessonsCount = $('.lesson-completed-checkbox:checked').length;

    // Previous button
    if (currentIndex === 0) {
        $('#prev-lesson-btn').css('display', 'none');
    } else {
        $('#prev-lesson-btn').css('display', 'inline-block');
    }

    // Next / Finish button
    if (currentIndex === totalLessons - 1) { // If on the last item
        if (completedLessonsCount === totalLessons) {
            $('#next-lesson-btn').html('Finish').css('display', 'inline-block');
        } else {
            $('#next-lesson-btn').css('display', 'none');
        }
    } else { // If not on the last item
        $('#next-lesson-btn').html('Next <i class="fas fa-angle-right"></i>').css('display', 'inline-block');
    }
}

function loadLesson(lessonElement) {
    if (!lessonElement || lessonElement.length === 0) {
        return;
    }

    $('.form-check').removeClass('item-active');
    lessonElement.closest('.form-check').addClass('item-active');

    hideSidebar();

    var lessonId = lessonElement.attr("data-lesson-id");
    var chapterId = lessonElement.attr("data-chapter-id");
    var courseId = lessonElement.attr("data-course-id");
    var type = lessonElement.attr("data-type");
            ...
            if (document.getElementById("vid1")) {
                ...
                player.on('timeupdate', function() {
                    ...
                    if (completionPercentage >= 80) {
                        ...
                        let type = $(".form-check.item-active .lesson-item").attr("data-type");

                        makeLessonComplete(lessonId, type, 1);

                        const checkbox = $(".lesson-completed-checkbox[data-lesson-id='" + lessonId + "'][data-type='" + type + "']");
...
$(document).ready(function () {
    ...
    $('#next-lesson-btn').off('click').on('click', function(e) {
        e.preventDefault();
        e.stopImmediatePropagation();
        ...
        const nextLesson = $(`.lesson-item[data-index='${nextIndex}']`);

        if (nextLesson.length) {
            const nextLessonChapterCollapse = nextLesson.closest('.accordion-collapse');
            if (!nextLessonChapterCollapse.hasClass('show')) {
                const chapterButton = nextLessonChapterCollapse.prev('.accordion-header').find('.accordion-button');
                if (chapterButton.length) {
                    chapterButton.trigger('click');
                    setTimeout(() => {
    ...
    $('#prev-lesson-btn').off('click').on('click', function(e) {
        e.preventDefault();
        e.stopImmediatePropagation();
        ...
        const prevLesson = $(`.lesson-item[data-index='${prevIndex}']`);

        if (prevLesson.length) {
            const prevLessonChapterCollapse = prevLesson.closest('.accordion-collapse');
            if (!prevLessonChapterCollapse.hasClass('show')) {
                const chapterButton = prevLessonChapterCollapse.prev('.accordion-header').find('.accordion-button');
                if (chapterButton.length) {
                    chapterButton.trigger('click');
                    setTimeout(() => {
