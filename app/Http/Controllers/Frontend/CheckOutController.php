...
class CheckOutController extends Controller
{
    ...
    function index()
    {
        ...
        foreach ($products as $product) {
            if (in_array($product->id, session()->get("enrollments"))) {
                return redirect()
                    ->route("cart")
                    ->with([
                        "messege" => __("Error occurred please try agin"),
                        "alert-type" => "error",
                    ]);
            }
        }
        ...
        $discountPercent = Session::has("offer_percentage")
            ? Session::get("offer_percentage")
            : 0;
        $discountAmount = ($cartTotal * $discountPercent) / 100;
        $total = currency($cartTotal - $discountAmount);
        $coupon = Session::has("coupon_code")
        ...
        $razorpay_calculate_charge = $this->calculate_payable_charge(
            $payable_amount,
            "razorpay",
        );
        ...
        $razorpay_credentials = (object) [
            ...
            "payable_with_charge" =>
                $razorpay_calculate_charge->payable_with_charge,
        ...
        $flutterwave_calculate_charge = $this->calculate_payable_charge(
            $payable_amount,
            "flutterwave",
        );
        ...
        $flutterwave_credentials = (object) [
            ...
            "payable_with_charge" =>
                $flutterwave_calculate_charge->payable_with_charge,
            "flutterwave_public_key" =>
                $payment_setting->flutterwave_public_key,
            "flutterwave_secret_key" =>
                $payment_setting->flutterwave_secret_key,
        ...
        $paystack_calculate_charge = $this->calculate_payable_charge(
            $payable_amount,
            "paystack",
        );
        ...
        $paystack_credentials = (object) [
            ...
            "payable_with_charge" =>
                $paystack_calculate_charge->payable_with_charge,
        ...
        $midtrans_credentials = (object) [
            ...
            "is_production" => filter_var(
                $midtrans_payment_settings["is_production"] ?? false,
